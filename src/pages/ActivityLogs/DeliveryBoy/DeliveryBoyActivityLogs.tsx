...
const AGENT_ACTIVITY_LOGS = gql`
  query GetActivityLogOfAgent($input: getActivityLogOfAgentInput!) {
    getActivityLogOfAgent(input: $input) {
      referenceDetails {
        _id
        ID
      }
...
const DeliveryBoyActivityLogs = () => {
  ...
  console.log("DELIVERY BOY ACTIVITIES = ", activities);

  const {
    data: activitiesData,
    loading: activitiesLoading,
    refetch: activitiesRefetch,
  } = useQuery(AGENT_ACTIVITY_LOGS, {
    variables: {
      input: {
        agentId: AgentID,
        date: selectedDate,
  ...
  useEffect(() => {
    if (activitiesData && activitiesData?.getActivityLogOfAgent) {
      setActivities(activitiesData?.getActivityLogOfAgent);
    }
  }, [activitiesData]);
  ...
  const items = [
    ...
    { text: "Detail Page", link: `/delivery-boys/view?id=${AgentID}` },
  ];
            ...
            <Col lg={12}>
              <Card>
                <CardHeader style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <h4 className="card-title m-0">{Agent || "Delivery Boy"}</h4>
...
export default DeliveryBoyActivityLogs;
